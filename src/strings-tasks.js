...
function isPalindrome(str) {
  ...
  return newStr === newStr.split('').reverse().toString().replaceAll(',', '');
}
...
function encodeToRot13(str) {
  ...
  const cif = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let newStr = '';
  for (let i = 0; i < str.length; i += 1) {
    if (
      str[i] !== ' ' &&
